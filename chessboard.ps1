...
$scriptblock = {
    ...
    function SovleMatrix {
        ...
        ## remove dupes
        $levels = $levels | Select-Object -Unique
        ## Do the Vowel check
        $finallevels = @()
        foreach ($level in $levels){
            $vcount = 0
    ...
    SovleMatrix -Letter $_ -MaxMoves 7
}# end scriptblock
...
$total = $list | ForEach-Object -Parallel $scriptblock

$final = 0 
$total | ForEach-Object { $final += $_}
$final